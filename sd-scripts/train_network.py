...
class EarlyStopper:
    """Val Loss の推移を監視し段階的な警告と早期停止を行う。

    判定ロジック:
      - Val Loss が前回計測値より ``threshold`` を超えて上昇 → 悪化カウント +1
      - Val Loss が前回計測値以下に戻る → カウントをリセットして正常化メッセージ
      - カウントが ``patience`` を超えた時点で停止フラグを立てる
      Train Loss は停止判定には使わず、警告メッセージの文脈情報として使用する。
    """
    ...
    def step(self, val_loss: float, train_loss: float) -> tuple[bool, str]:
        """
        Returns
        -------
        (should_stop, message)
          should_stop: True のとき呼び出し側は学習ループを break する
          message    : ログ出力用文字列。空文字のときはログ不要
        """
        msg = ""
        should_stop = False
        ...
        if self._prev_val_loss is None:
            ...
            return False, (
                f"[EarlyStopping] 初回計測  "
                f"val_loss={val_loss:.6f}  train_loss={train_loss:.6f}  監視を開始します。"
            )

        val_improved = val_loss <= self._prev_val_loss + self.threshold
        train_falling = (
            self._prev_train_loss is not None and train_loss < self._prev_train_loss
        )
        ...
        if val_improved:
            ...
            # Val Loss が悪化
            self._count += 1
            n, p = self._count, self.patience

            if n >= p:
                # 停止（カウントが patience に達した）
                should_stop = True
                if train_falling:
                    msg = (
                        f"[緊急停止] Val Loss が {p} 回連続で上昇しました  "
                        f"val_loss: {self._prev_val_loss:.6f} → {val_loss:.6f}  "
                        f"train_loss: {train_loss:.6f}  "
                        f"Train Loss は下降中のため過学習と判断し学習を停止します。"
                    )
                else:
                    msg = (
                        f"[緊急停止] Val Loss が {p} 回連続で上昇しました  "
                        f"val_loss: {self._prev_val_loss:.6f} → {val_loss:.6f}  "
                        f"train_loss: {train_loss:.6f}  "
                        f"学習の収束失敗と判断し学習を停止します。"
                    )
            elif n == 1:
                # 最初の悪化 → 注意レベル
                if train_falling:
                    msg = (
                        f"[注意] Val Loss が上昇しています ({n}/{p})  "
                        f"val_loss: {self._prev_val_loss:.6f} → {val_loss:.6f}  "
                        f"train_loss: {train_loss:.6f}  "
                        f"Train Loss は下降中のため過学習の初期兆候の可能性があります。引き続き監視します。"
                    )
                else:
                    msg = (
                        f"[注意] Val Loss が上昇しています ({n}/{p})  "
                        f"val_loss: {self._prev_val_loss:.6f} → {val_loss:.6f}  "
                        f"train_loss: {train_loss:.6f}  引き続き監視します。"
                    )
            else:
                # 中間 (2 ≤ n < p) → 警告レベル
                if train_falling:
                    msg = (
                        f"[警告] Val Loss の上昇が継続しています ({n}/{p})  "
                        f"val_loss: {self._prev_val_loss:.6f} → {val_loss:.6f}  "
                        f"train_loss: {train_loss:.6f}  "
                        f"Train Loss は下降中のため過学習の可能性が高まっています。"
                    )
                else:
                    msg = (
                        f"[警告] Val Loss の上昇が継続しています ({n}/{p})  "
                        f"val_loss: {self._prev_val_loss:.6f} → {val_loss:.6f}  "
                        f"train_loss: {train_loss:.6f}  "
                        f"学習の発散またはデータ不足の可能性があります。"
                    )
        self._prev_val_loss = val_loss
        self._prev_train_loss = train_loss
        return should_stop, msg
...
class NetworkTrainer:
    ...
    def train(self, args):
        ...
        # EarlyStopper の初期化
        _es_enabled = getattr(args, 'early_stopping', False)
        _es_mode    = getattr(args, 'early_stopping_mode', 'epoch')  # 'epoch' or 'step'
        _es_patience  = getattr(args, 'early_stopping_patience', 3)
        _es_threshold = getattr(args, 'early_stopping_threshold', 0.01)
        early_stopper: Optional[EarlyStopper] = (
            EarlyStopper(patience=_es_patience, threshold=_es_threshold)
            if _es_enabled else None
        )
        _es_should_stop = False  # エポック末の break 用フラグ
        ...
        # EarlyStopper: validation データが空なら無効化
        if early_stopper is not None and validation_steps == 0:
            logger.warning(
                '[EarlyStopping] 有効化されていますが validation データセットが空です。'
                'EarlyStopping は無効化されます。'
                'validation_split を設定してください。'
            )
            early_stopper = None
        ...
        for epoch in range(epoch_to_start, num_train_epochs):
            ...
            for step, batch in enumerate(skipped_dataloader or train_dataloader):
                ...
                if _es_should_stop or global_step >= args.max_train_steps:
                    break
            ...
            if _es_should_stop:
                accelerator.print('[EarlyStopping] 学習を早期停止しました。')
                break
...
def setup_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--early_stopping",
        action="store_true",
        help="Enable early stopping based on validation loss / Val Lossによる早期停止を有効化する",
    )
    parser.add_argument(
        "--early_stopping_mode",
        type=str,
        default="epoch",
        choices=["epoch", "step"],
        help="Trigger early stopping per epoch or per step (requires validate_every_n_steps) / エポックごとまたはステップごとに早期停止を判定する",
    )
    parser.add_argument(
        "--early_stopping_patience",
        type=int,
        default=3,
        help="Number of consecutive val loss increases before stopping / 連続して Val Loss が悪化した回数がこの値を超えると停止する",
    )
    parser.add_argument(
        "--early_stopping_threshold",
        type=float,
        default=0.01,
        help="Minimum increase in val loss to count as deterioration / Val Loss がこの値を超えて上昇した場合に悪化とみなす",
    )
    return parser
