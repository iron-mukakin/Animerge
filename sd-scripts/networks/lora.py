...
# # # from library.sdxl_original_unet import SdxlUNet2DConditionModel  # disabled: not needed for Anima  # disabled: not needed for Anima  # disabled: not needed for Anima
...
def _component_from_lora_name(lora_name: str) -> str:
    """lora_name からコンポーネントカテゴリを返す。

    merge.py の component_category() と同等のロジックを lora.py 内に inline 実装。
    外部 import を避けて sd-scripts への干渉を最小化する。
    """
    n = lora_name.lower()
    if any(t in n for t in ("attn", "attention", "to_q", "to_k", "to_v", "to_out", "self_attn", "cross_attn")):
        return "Attention"
    if any(t in n for t in ("mlp", "ff", "feed_forward", "ffn", "proj_in", "proj_out")):
        return "MLP"
    if any(t in n for t in ("norm", "ln_", "layernorm", "groupnorm")):
        return "Norm"
    if any(t in n for t in ("resnet", "resblock", "resnets", "skip_connection")):
        return "ResNet"
    if any(t in n for t in ("time_embed", "timestep", "temb", "time_embedding")):
        return "Timestep"
    return "Other"
...
def create_network(
    ...
    **kwargs,
    ...
    # Anima does not use SDXL UNet; is_sdxl is always False
    is_sdxl = False
    ...
    if not _is_anima_block_lr and (block_dims is not None or block_lr_weight is not None):
    ...
    # anima_matrix_scales: Matrix モード用の (Block, Component) 別スケール辞書
    anima_matrix_scales_str = kwargs.get("anima_matrix_scales", None)
    if anima_matrix_scales_str is not None:
        import json as _json
        try:
            scales = _json.loads(anima_matrix_scales_str)
            network.set_anima_matrix_scales(scales)
        except Exception as exc:
            logger.warning(f"anima_matrix_scales parse error: {exc} — falling back to block-level LR")
...
class LoRANetwork(torch.nn.Module):
    ...
    UNET_TARGET_REPLACE_MODULE = ["Block", "LLMAdapterTransformerBlock"]  # Anima DiT
    UNET_TARGET_REPLACE_MODULE_CONV2D_3X3 = []  # Anima DiT has no Conv2D 3x3 blocks
    ...
    def set_anima_matrix_scales(self, scales: dict) -> None:
        """Matrix モード用の (Block_Component) 別スケール辞書を設定する。

        scales 例: {"Input_Attention": 0.8, "Middle_MLP": 0.5, ...}
        設定後は get_lr_weight_for_lora() が block_lr_weight より優先される。
        """
        self.anima_matrix_scales = scales
        logger.info(f"anima_matrix_scales: {len(scales)} groups loaded")

    def get_lr_weight_for_lora(self, lora_name: str) -> float:
        """lora_name から直接 LR スケールを返す。anima_matrix_scales 優先。

        anima_matrix_scales が設定されている場合:
            (Block カテゴリ, Component) のキーでスケールを参照する。
            マッチしない場合は 1.0。
        未設定の場合:
                    ...
                    if self.anima_matrix_scales:
                        ...
                        for (b_idx, comp), group_loras in group_to_lora.items():
                            ...
                            lr_descriptions.extend(
                                [label + (" " + d if d else "") for d in descriptions]
                            )
                    else:
                        ...
                        for idx, block_loras in block_idx_to_lora.items():
                            label = f"anima_block{idx}" if idx >= 0 else "anima_block_other"
                            params, descriptions = assemble_params(
                                block_loras,
