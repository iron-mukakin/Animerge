"""lora_train.py (GUI) に `qwen_image_vae_2d` チェックボックスを追加し、
コマンド生成・設定保存/読込に連携させるパッチ。

実行例:
    python apply_fix_lora_train_gui.py lora_train.py
"""
...
DEFAULT_RELATIVE_PATH: str = "lora_train.py"
...
WIDGET_SEARCH: str = '''    ttk.Checkbutton(lf2, text="vae_disable_cache", variable=s.vae_disable_cache).grid(
        row=1, column=0, sticky=tk.W, padx=8, pady=3)
'''
WIDGET_REPLACEMENT: str = '''    ttk.Checkbutton(lf2, text="vae_disable_cache", variable=s.vae_disable_cache).grid(
        row=1, column=0, sticky=tk.W, padx=8, pady=3)
    ttk.Checkbutton(lf2, text="qwen_image_vae_2d", variable=s.qwen_image_vae_2d).grid(
        row=1, column=1, sticky=tk.W, padx=8, pady=3)
'''

COMMAND_BUILD_SEARCH: str = '''        (s.cpu_offload_checkpointing,   "--cpu_offload_checkpointing"),
        (s.vae_disable_cache,           "--vae_disable_cache"),
    ]
'''
COMMAND_BUILD_REPLACEMENT: str = '''        (s.cpu_offload_checkpointing,   "--cpu_offload_checkpointing"),
        (s.vae_disable_cache,           "--vae_disable_cache"),
